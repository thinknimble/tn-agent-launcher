...
class AgentInstance(AbstractBaseModel):
    ...
    target_url = models.URLField(null=True, blank=True, help_text="Optional base URL for the model API, if applicable")
    agent_type = models.CharField(max_length=50, default=AgentTypeChoices.CHAT, choices=AgentTypeChoices.choices, help_text="Type of agent, e.g., 'chat', 'one-shot', etc.")
    ...
    async def agent(self):
        ...
        return Agent(name=self.friendly_name, model=self.raw_agent, output_type=str, system_prompt=system_prompt)

...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f"User {email} not found. Please create a user first."))
            return
        ...
        try:
            execution = schedule_agent_task_execution(
                agent_task_id=task.id,
                force_execute=True
            )
            
            if execution:
                self.stdout.write(self.style.SUCCESS(f"✓ Task scheduled for execution"))
                ...
                if execution.status == 'completed':
                    ...
                    if execution.output_data:
                        self.stdout.write(f"  Response: {execution.output_data.get('result', 'No result')}")

...
class Integration(AbstractBaseModel):
    ...
    _app_credentials = EncryptedTextField(blank=True, default="", help_text="Credentials required for app-based integrations, google console app or s3 keys")
    _oauth_credentials =  EncryptedTextField(blank=True, default="", help_text="Credentials required for OAuth-based integrations, including access and refresh tokens")
    webhook_url = models.URLField(blank=True, null=True, help_text="URL for webhook-based integrations")
    webhook_secret = EncryptedTextField(max_length=255, blank=True, default="", help_text="Secret for validating webhook payloads")

    # decide if this is needed
    agent_tasks = models.ManyToManyField('agent.AgentTask', blank=True, related_name='integrations', help_text="Agent tasks that can use this integration")

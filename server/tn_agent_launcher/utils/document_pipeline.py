...
class DocumentProcessor:
    ...
    def configure_for_pdfs(
        self,
        contains_images: bool = True,
        extract_images_as_text: bool = True
    ) -> None:
    ...
    def _detect_document_type(self, file_path: str) -> DocumentType:
        ...
        type_mapping = {
            # DOCX formats
            'docx': DocumentType.DOCX, 'dotx': DocumentType.DOCX, 
            'docm': DocumentType.DOCX, 'dotm': DocumentType.DOCX,
            
            # PPTX formats  
            'pptx': DocumentType.PPTX, 'potx': DocumentType.PPTX, 
            'ppsx': DocumentType.PPTX, 'pptm': DocumentType.PPTX,
            'potm': DocumentType.PPTX, 'ppsm': DocumentType.PPTX,
            ...
            'png': DocumentType.IMAGE, 'tif': DocumentType.IMAGE,
            'tiff': DocumentType.IMAGE, 'bmp': DocumentType.IMAGE,
            ...
            # AsciiDoc
            'adoc': DocumentType.ASCIIDOC, 'asciidoc': DocumentType.ASCIIDOC,
            'asc': DocumentType.ASCIIDOC,
            
            # XML formats
            'xml': DocumentType.XML_JATS, 'nxml': DocumentType.XML_JATS,
            ...
            # Audio
            'wav': DocumentType.AUDIO, 'mp3': DocumentType.AUDIO,
        }
        
        return type_mapping.get(suffix, DocumentType.PDF)
    
    def _process_images_in_document(self, doc, md_content: str) -> tuple[str, str, ProcessingMetadata]:
        ...
        for pic in doc.pictures:
            ...
            if self.image_config.replace_images_with_descriptions:
                if all_text.strip() == "" and pic.image:
                    ...
                    replacement = all_text if all_text.strip() else "_No caption or annotations found._"
    ...
    def process_document(self, file_path: str) -> ProcessingResult:
        ...
        try:
            ...
            if doc.pictures and self.image_config.extract_image_descriptions:
                processed_md, html_content, metadata = self._process_images_in_document(doc, md_content)
            else:
                processed_md = md_content
                html_content = ""
                metadata = ProcessingMetadata()
